...
一款 Minecraft Java 版数据包和材质包的组合，用带有自定义 NBT 标记的下界之星填充玩家背包中的受保护槽位，并将这些下界之星显示为自定义图片。内部命名空间：`slotlock`。
...
- 数据包每 20 tick 运行一次轻量审计；只有检测到库存发生变化的玩家才会执行 35 个受保护槽位的检查。每个正常槽位只检查一次标记下界之星状态，异常时才进入物品恢复分支。
- 如果普通物品被交换进被锁定槽位，会先保存完整物品数据并将物品掉落到玩家脚下，再恢复下界之星。标记下界之星只会在库存活动后的短时间内于玩家附近清理，不再持续扫描整个维度的掉落物。
...
- 数据包生成的标记下界之星带有 `CustomModelData:1`，材质包通过 `assets/minecraft/models/item/nether_star.json` 的模型覆盖，将它替换为：

```text
one-item-resourcepack/assets/slotlock/textures/item/blocked_item.png
```

- 建议保持 PNG 为 `16×16`，并保留文件名和路径不变。材质包不会改变槽位的实际点击区域，只改变标记下界之星的显示效果。
